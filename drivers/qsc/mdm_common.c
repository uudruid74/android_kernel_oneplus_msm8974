...
// #define CONFIG_STATUS_IRQ_BY_LEVEL_TRIG
#define CONFIG_STATUS_IRQ_WAKEUP_ENA
// #define CONFIG_ABNORMAL_RESET_SKIP_RAMPDUMP

extern int subsys_set_flag_skip_ramdump(struct subsys_device *dev, bool flag);
...
void mdm_raise_mdm_errfatal(void)
{
...
static void mdm2ap_status_check(struct work_struct *work)
{
	...
	subsys_set_flag_skip_ramdump(mdev->mdm_subsys_dev, false);
	/*
	...
	if (!mdm_drv->disable_status_check) {
		if (gpio_get_value(MDM_GPIO(MDM2AP_STATUS)) == 0) {
			pr_info("%s: MDM2AP_STATUS did not go high on mdm id %d\n",
				   __func__, mdev->mdm_data.device_id);
//			subsys_set_flag_skip_ramdump(mdev->mdm_subsys_dev ,true);
			mdm_start_ssr(mdev);
		}
	}
	pr_info("%s:  %d\n",__func__, __LINE__);
				...
				unsigned long arg)
{
	...
	case RAM_DUMP_DONE:
		...
		if (status)
			...
			if (sec_debug_is_enabled()) {
				panic("external_modem QSC err-fatal");
			}
#endif
		}
	...
	case GET_DLOAD_STATUS:
		...
		break;
	case IMAGE_UPGRADE:
		pr_info("%s Image upgrade ioctl recieved\n", __func__);
...
#ifdef CONFIG_ABNORMAL_RESET_SKIP_RAMPDUMP
		pr_info("%s: forced skip_rampdump\n", __func__);
		mdm_drv->need_clean_reset = true;
		subsys_set_flag_skip_ramdump(mdev->mdm_subsys_dev, true);
#endif
				...
				const struct subsys_desc *crashed_subsys)
{
	...
	cancel_delayed_work(&mdev->mdm2ap_status_check_work);
	if (want_dumps) {
		/* DEBUG: Disable the IPC logging */
		//msm_hs_disable_ipc_log();
				...
				struct mdm_device *mdev)
{
	...
	if (ret < 0) {
		pr_info("%s: vddmin_modes not set.\n", __func__);
		return -ENXIO;
	} else {
	...
	if (ret < 0) {
		pr_info("%s: vddmin_drive_strength not set.\n", __func__);
		return -ENXIO;
	}
				...
				struct mdm_device *mdev)
{
	...
	if (ret < 0)
		pr_info("%s: ramdump_timeout not set.\n", __func__);
	...
	if (ret < 0)
		pr_info("%s: sysmon_subsys_id not set.\n", __func__);
...
#ifdef CONFIG_STATUS_IRQ_WAKEUP_ENA
	/* enable wake up irq */
...
#endif
	/*
	 * Don't pull AP2MDM_PMIC_PWR_EN high right now.
	 * We'll assert it on 'mdm_do_first_power_on'
	 */
	if (GPIO_IS_VALID(MDM_GPIO(AP2MDM_PMIC_PWR_EN))) {
...
static int  mdm_modem_probe(struct platform_device *pdev)
{
	...
	pr_info("%s 4 : %d\n", __func__, ret);
	if (ret) {
		pr_info("%s: 1 registered mdm id %d\n",
			   __func__, mdev->mdm_data.device_id);
		mdm_deconfigure_ipc(mdev);
		goto end;
	} else {
		pr_err("%s: 2 registered mdm id %d\n",
			   __func__, mdev->mdm_data.device_id);
...
static const struct of_device_id mdm_match_table[] = {
