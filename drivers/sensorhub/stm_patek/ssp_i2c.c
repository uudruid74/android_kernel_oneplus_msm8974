...
void set_magnetic_cal_with_folder_state(struct ssp_data *data, bool bEnable)
{
	int iRet = 0;

	struct ssp_msg *msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	msg->cmd = MSG2SSP_AP_MCU_SET_MAGNETIC_CAL;
	msg->length = 1;
	msg->options = AP2HUB_WRITE;
	msg->buffer = (char*) kzalloc(1, GFP_KERNEL);
	msg->free_buffer = 1;
	...
	iRet = ssp_spi_async(data, msg);
	if (iRet != SUCCESS) {
		pr_err("[SSP]: %s - SENSOR_MAGNETIC_CAL fail %d\n",
				__func__, iRet);
		return;
	}
	pr_info("[SSP] magneticsensor calibration with folder state En : %u\n", bEnable);
}
#endif
