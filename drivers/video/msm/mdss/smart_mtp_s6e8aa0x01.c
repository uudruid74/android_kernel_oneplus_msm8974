...
#include "smart_mtp_2p2_gamma_s6e8aa0x01.h"
...
static int translate_table_108_to102[4] = {122, 134, 147, 159};
...
static void gamma_init(struct SMART_DIM *pSmart, char *str, int size)
{
	...
	if (bl_level >= gamma_curve_change_point[1]) {
		...
		if ((bl_level < (AOR_FIX_CD + 10)) &&
			...
			if(bl_level >=102 && bl_level <=109){
				if (bl_level >= 108)
					aid_bl_level = translate_table_108_to102[3];
				else if (bl_level >= 106)
					aid_bl_level = translate_table_108_to102[2];
				else if (bl_level >= 104)
