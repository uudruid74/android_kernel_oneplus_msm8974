/*
 * LED driver for Maxim MAX77888 - leds-max77888.c
 *
...
#define DEBUG_MAX77888(fmt, args...) pr_err(fmt, ##args)
#else
#define DEBUG_MAX77888(fmt, args...) do{}while(0)
#endif
...
#ifdef CONFIG_LEDS_SEPERATE_MOVIE_FLASH
static ssize_t max77888_show_movie_brightness(struct device *dev,
				     struct device_attribute *devattr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct max77888_led_data *led_data = container_of(led_cdev, struct max77888_led_data, led);

	return sprintf(buf, "%d\n", led_data->movie_brightness);
}

static ssize_t max77888_store_movie_brightness(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = max77888_set_bits(led_data->i2c, reg_led_current[led_data->data->id],
...
static DEVICE_ATTR(movie_brightness, S_IWUSR|S_IWGRP|S_IRUGO,
	max77888_show_movie_brightness, max77888_store_movie_brightness);
...
static int max77888_led_get_en_value(struct max77888_led_data *led_data, int on)
{
	...
	if (on)
		...
		if (flash_torch_en) {
			...
			if (ret)
			    ...
			    DEBUG_MAX77888("[LED] %s: max77888_flash_en set 0\n", __func__);
			...
			if (ret)
			    ...
			    DEBUG_MAX77888("[LED] %s: max77888_torch_en set 0\n", __func__);
	...
	DEBUG_MAX77888("[LED] %s: led mode: 0x%x, on: %d\n", __func__, mode, on);

	return mode;
}

int max77888_led_en(int onoff, int mode)
{
	int ret = 0;
	...
	if (flash_torch_en) {
	    if (onoff) { 	/* enable */
		if (mode) { 	/* flash */
			DEBUG_MAX77888("[LED] %s: max77888_flash_en set 1\n", __func__);
			gpio_direction_output(led_flash_en, 1);
		} else { 	/* torch */
			DEBUG_MAX77888("[LED] %s: max77888_torch_en set 1\n", __func__);
...
static void led_set(struct max77888_led_data *led_data)
{
	...
	/* Turn on LED */
	DEBUG_MAX77888("[LED] %s : LED turns on\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (count == size) {
		...
		DEBUG_MAX77888("[LED] %s : led_cdev->brightness %d\n",
...
static int max77888_led_probe(struct platform_device *pdev)
{
	...
	struct max77888_led_platform_data *pdata = max77888_pdata->led_data;
	struct max77888_led_data *led_data;
	struct max77888_led *data;
	struct max77888_led_data **led_datas;

	pr_err("[zealottt] max77888_led_probe\n");
	...
	if (led_torch_en && led_flash_en) {
	    DEBUG_MAX77888("[LED] %s, led_torch %d, led_flash %d\n", __func__,
	...
	for (i = 0; i != pdata->num_leds; ++i) {
		...
		led_data->led.max_brightness = reg_led_timer[data->id] == MAX77888_LED_REG_FLASH_TIMER
